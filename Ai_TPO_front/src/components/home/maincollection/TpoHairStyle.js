...
const TpoHairStyle = () => {
    ...
    // 단계 정의 - 상황 선택 단계 제거
    const steps = ['얼굴 사진 업로드', '얼굴형 선택', '현재 헤어 길이'];
        ...
        try {
            ...
            */
            ...
            setTimeout(() => {
                ...
                setSimulationResult({
                    style_info: {
                        description: `${selectedFaceShape} 얼굴형과 ${selectedHairLength} 길이에 맞는 헤어스타일입니다.`,
    ...
    const ResultDialog = () => {
        const getFaceShapeName = () => {
            const faceShape = faceShapes.find(f => f.id === selectedFaceShape);
            return faceShape ? faceShape.name : '';
        };

        const getHairLengthName = () => {
            const hairLength = hairLengths.find(h => h.id === selectedHairLength);
            return hairLength ? hairLength.name : '';
        };
                        ...
                        <Grid item xs={12} md={6}>
                            {isLoading ? (
                                <Box sx={{
                                    ...
                                    <Typography variant="body2" color="text.secondary">
                                        {getFaceShapeName()} 얼굴형과 {getHairLengthName()}에 적합한 헤어스타일을 생성하고 있습니다.
                                    </Typography>
                                ...
                                <Box>
                                    ...
                                    <Box sx={{ mt: 3 }}>
                                        <Typography variant="h6" gutterBottom>
                                            {getFaceShapeName()} + {getHairLengthName()} 추천 스타일
                                        </Typography>
                                            ...
                                            <Box sx={{ mt: 2 }}>
                                                <Typography variant="body1" paragraph>
                                                    {simulationResult.style_info.description ||
                                                        `${getFaceShapeName()} 얼굴형에 어울리는 ${getHairLengthName()} 스타일입니다.`}
                                                </Typography>
                    ...
                    {simulationResult && simulationResult.style_info && (
                        <Box sx={{ mt: 4, p: 2, bgcolor: 'rgba(156, 39, 176, 0.05)', borderRadius: 2 }}>
                            ...
                            <Typography variant="body2">
                                얼굴형: <strong>{getFaceShapeName()}</strong> |
                                헤어 길이: <strong>{getHairLengthName()}</strong>
                            </Typography>
                        </Box>
                    )}
                </DialogContent>
